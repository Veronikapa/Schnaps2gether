...
public class Spiel2RundenEnde {
    ...
    public void Auspielen(Karte kartesprint1S1)
    {
        //Ausgespielte Karte wird aus Hand von S1 entfernt
        s1.Hand.remove(kartesprint1S1);
        s1.setIstdran(false);
    }
...
/*
    ...
    private Karte findeAuszuspielendeKarte(Karte kartesprint1S1) {
        Karte zuspielen;//Spieler1 hat bereits Karte ausgespielt, entscheide basierend auf dieser Karte
        if (kartesprint1S1 != null) {

            if(zugedreht)
                zuspielen =  SpieleComputerKarteWennS1Zugedreht(kartesprint1S1);

            else
                zuspielen = SpieleComputerKarte(kartesprint1S1);
        }
...
/*
    ...
    private Karte SpieleComputerKarte(Karte kartesprint1S1) {

        //Habe ich Karte mit selber Farbe
        Karte karteMitSelberFarbe = computerKannFaerbeln(kartesprint1S1);

        //Habe ich Trumph mit dem ich stechen könnte, wenn S1 keinen Trumpf gespielt hat
        Karte trumpfKarte = null;
        if(aufgedeckterTrumpf.getFarbe()!= kartesprint1S1.getFarbe())
        {
            trumpfKarte = computerHatTrumpf();
        }

        //Habe karteMitSelberFarbe die höher als die von s1 gespielte ist, spiele diese
        if(karteMitSelberFarbe !=null && karteMitSelberFarbe.getPunkte() > kartesprint1S1.getPunkte())
        {
...
/*
    ...
    private Karte SpieleComputerKarteWennS1Zugedreht(Karte kartesprint1S1) {
            //Habe ich Karte mit selber Farbe
            Karte karteMitSelberFarbe = computerKannFaerbeln(kartesprint1S1);
...
/*
    ...
    */
    ...
    private Karte computerKannFaerbeln(Karte kartesprint1S1) {

        Karte karteZumFaerbeln = null;
        for (int i =0; i < s2.Hand.size(); i++)
        {
            if(s2.Hand.get(i).getFarbe()== kartesprint1S1.getFarbe())
            {
    ...
    public void ZugAuswerten(Karte kartesprint1S1, Karte kartesprint1S2)
    {
        ...
        {
            if (kartesprint1S1.getFarbe() == trumpf && kartesprint1S2.getFarbe() == trumpf)
            {
                if(kartesprint1S1.getPunkte()> kartesprint1S2.getPunkte())
                {
                    s1.Gestochen.add(kartesprint1S1);
                    s1.Gestochen.add(kartesprint1S2);

                    s1.setPunkte(s1.getPunkte()+ kartesprint1S1.getPunkte()+ kartesprint1S2.getPunkte());
                ...
                {
                    s2.Gestochen.add(kartesprint1S1);
                    s2.Gestochen.add(kartesprint1S2);

                    s2.setPunkte(s2.getPunkte()+ kartesprint1S1.getPunkte()+ kartesprint1S2.getPunkte());
            ...
            else if(kartesprint1S1.getFarbe() != trumpf && kartesprint1S2.getFarbe() != trumpf)
            {
                if(kartesprint1S1.getFarbe().equals(kartesprint1S2.getFarbe())) {
                    if (kartesprint1S1.getPunkte() > kartesprint1S2.getPunkte()) {
                        s1.Gestochen.add(kartesprint1S1);
                        s1.Gestochen.add(kartesprint1S2);
            ...
            {
                if(kartesprint1S1.getFarbe() == trumpf)
                {
                    s1.Gestochen.add(kartesprint1S1);
                    s1.Gestochen.add(kartesprint1S2);
